...
class Analysis:
    ...
    def __stationarity_rolling_window(
        self, df: pd.DataFrame, name: str, window_size: int
    ):
        ...
        fig.add_trace(
            go.Scatter(x=df["date_datetime"], y=df[name], mode="lines", name="Original")
        )
        fig.add_trace(
            go.Scatter(
                x=df["date_datetime"],
        ...
        fig.add_trace(
            go.Scatter(
                x=df["date_datetime"],
                y=df["rolling_standard_deviation"],
                mode="lines",
                name="Rolling Std",
            )
        )
        ...
        fig.update_layout(
            title="Rolling Window Analysis", xaxis_title="Date", yaxis_title=name
        )
    ...
    def acf_plot(self, df: pd.DataFrame, name: str, lags: int):
        ...
        fig.add_trace(
            go.Scatter(
                x=list(range(lags + 1)), y=acf_values, mode="lines+markers", name="ACF"
            )
        )
    ...
    def decomposition_time_series(
        self, df: pd.DataFrame, name: str, period: int, title: str
    ):
        """
    ...
    def forecast_component(self, train: pd.DataFrame, test: pd.DataFrame, order: tuple):
        ...
        forecast_dates = pd.date_range(
            start=train["date_datetime"].iloc[-1], periods=forecast_steps, freq="D"
        )
        forecast_df = pd.DataFrame(
            {"date_datetime": forecast_dates, "forecast": predictions.predicted_mean}
        )
